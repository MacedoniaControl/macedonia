-- Prueba de cilindros · PASO 1 de 3: crea la prueba en un esquema aparte
-- (prueba_tmp), que la app no puede llamar. Corre como cada usuario real y al
-- final DESHACE TODO: no deja movimientos, conteos ni números gastados.
--   Paso 2 (consulta aparte):  select * from prueba_tmp.probar_cilindros();
--   Paso 3 (consulta aparte):  drop schema prueba_tmp cascade;
create schema if not exists prueba_tmp;
grant usage on schema prueba_tmp to authenticated;
-- Entra como un usuario (o vuelve a postgres) solo dentro de esta prueba.
create or replace function prueba_tmp.como(p uuid, p_rol text default 'authenticated')
returns void language plpgsql as $c$
begin
  perform set_config('role', p_rol, true);
  perform set_config('request.jwt.claims', json_build_object('sub', p::text, 'role', 'authenticated')::text, true);
end $c$;
grant execute on function prueba_tmp.como(uuid, text) to authenticated;

create or replace function prueba_tmp.probar_cilindros()
...
-- ======== 1. QUIÉN VE QUÉ
  ...
  begin perform prueba_tmp.como(V);
  ...
  begin perform prueba_tmp.como(T2);
    select count(*) into x from public.cilindros_saldo where empresa_id = 'sumigases';
    perform prueba_tmp.como(A, 'postgres');
...
perform prueba_tmp.como(A, 'postgres'); l0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'lleno'); c0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'en_cliente'); v0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'vacio');
...
perform prueba_tmp.como(A, 'postgres'); l0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'lleno'); c0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'en_cliente');
...
perform prueba_tmp.como(A, 'postgres'); l0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'lleno'); v0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'vacio'); c0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'en_cliente');
...
perform prueba_tmp.como(A, 'postgres'); l0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'lleno'); v0 := public.saldo_cilindro('sumigases', 'OXIGENO', 'vacio');
...
-- ======== 7. FECHAS
  ...
  begin perform prueba_tmp.como(A, 'postgres');
...
revoke execute on function prueba_tmp.probar_cilindros() from public, anon, authenticated;
...
select count(*) as funcion_creada from pg_proc p join pg_namespace s on s.oid = p.pronamespace
 where s.nspname = 'prueba_tmp' and p.proname = 'probar_cilindros';
